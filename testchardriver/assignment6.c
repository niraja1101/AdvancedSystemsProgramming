...
int e2_open(struct inode *inode, struct file *filp)
{
    ...
    if (devc->mode == MODE1) {
        devc->count1++;
	printk(KERN_INFO "count1 %d",devc->count1);
        up(&devc->sem1);
	printk(KERN_INFO "Opening Device");
 	//printk(KERN_INFO "%d", devc->mode);
        down_interruptible(&devc->sem2);
	printk(KERN_INFO "check point");
        return 0;
    }
    else if (devc->mode == MODE2) {
        devc->count2++;
	printk(KERN_INFO "count2 %d",devc->count2);
    }
    up(&devc->sem1);
    return 0;
}
...
int e2_release(struct inode *inode, struct file *filp)
{
    struct e2_dev *devc = container_of(inode->i_cdev, struct e2_dev, cdev);
    down_interruptible(&devc->sem1);
    printk(KERN_INFO "Closing Device,place1");
    if (devc->mode == MODE1) {
        devc->count1--;
        if (devc->count1 == 1)
            wake_up_interruptible(&(devc->queue1));
		printk(KERN_INFO "Closing Device,place2");	
			up(&devc->sem2);
    }
    else if (devc->mode == MODE2) {
        devc->count2--;
